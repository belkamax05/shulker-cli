...
echo-verbose-debug "$prefix. File: $(format-args "$fromRelativePath"), Dir: $(format-args "$fileDir"). Rest args: $@"

echo-verbose-debug "$prefix FromDir: $(format-args "$directoryFrom"), ToDir: $(format-args "$directoryTo")"
